...
- **Basemaps follow the theme.** The three basemap buttons keep their labels; only their target changes: Classique `liberty` → `fiord`, Gris `positron` → `dark`, Satellite unchanged (the imagery is already dark). Resolution goes through `mapStyleFor()` in `main.js`; no page reads `MAP_STYLES` directly. A theme change dispatches a `themechange` event on `document`, and each map replays `setStyle` then rebuilds its own sources and layers, through `replayMapStyle()` in `main.js` — a non-diff `setStyle` followed by `style.load`, the only reliable signal that the new style is in place.
